...
class TestRecent:
    def test_order_created_desc_then_memory_id_asc(self, repo):
        ...
        ids = [r.memory_id for r in repository.recent(limit=10)]
    ...
    def test_live_filter_before_limit(self, repo):
        ...
        ids = [r.memory_id for r in repository.recent(limit=1)]
    ...
    def test_filters_topic_catalog_window(self, repo):
        ...
        assert [r.memory_id for r in repository.recent(
            limit=10, filters=RecentFilters(topic="sqlite"))] == ["m3", "m1"]
        assert [r.memory_id for r in repository.recent(
            limit=10, filters=RecentFilters(catalog="research"))] == ["m3"]
        assert [r.memory_id for r in repository.recent(
            limit=10, filters=RecentFilters(since_ms=150, until_ms=250))] == ["m2"]
