...
diary/codec.py + diary/store.py, which are production-proven.
"""
...
_FLOAT_FIELDS = frozenset(
    {"period_start", "period_end", "created_at", "updated_at", "deleted_at", "score"}
)
...
class RedisMemoryRepository:
    ...
    async def bm25_search(
        ...
        """BM25 over summary|content (Step 04 §6.2). Terms are OR-joined:
        lexical recall wants any-keyword-matches, not all-words-present
        (march7 lesson — AND semantics silently degrade hybrid to KNN-only).
        """
        ...
        if not terms:
            return []
        expr = self._filter_expr(brain_id, filters)
        query = f"({expr}) (@summary|content:({' | '.join(terms)}))"
        try:
            reply = await self._redis.execute_command(
                "FT.SEARCH", self._keys.index_name,
                query,
            ...
            return []
        return self._hits(reply, score_in_fields=False, has_scores=True)
