...
- Host port is `REDIS_PORT` (default **1906**; `.env` may override).
  ...
  Beware a stale `.env`: if `REDIS_PORT` there points where no Redis
  listens, integration tests **skip** ("Redis not reachable") instead of
  failing — check `uv run pytest -rs` before trusting a green run.
