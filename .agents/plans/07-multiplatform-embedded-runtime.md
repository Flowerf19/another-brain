...
| TASK-084 | Implement `another-brain doctor`: package/model hashes, tokenizer/profile, SQLite bootstrap/readonly invariants, schema/integrity/FTS/extension or fallback, isolated write/search/delete probe, paths, actionable per-item results, plus an explicit OS-support verdict line from the measured wheel matrix. Landed with 8 tests + fresh-profile smoke. | ✅ | 2026-08-06 |
| TASK-085 | Forced-NumPy-fallback E2E switch (env/CLI) so CI and users can exercise the non-sqlite-vec path honestly; wired into the wheel gates as a second pass. Landed: `BRAIN_DISABLE_SQLITE_VEC`, 19 tests, full suite 631 green. | ✅ | 2026-08-06 |
| TASK-086 | CI `wheel-gate` matrix { ubuntu-22.04, ubuntu-24.04, macos-14, windows-2022 } × Python 3.12–3.14 running `installer/<os>/` gates + unit tests + clean-tree + one forced-fallback pass per OS; first real execution of the Windows `.ps1`. Green across all cells (run 31089526222); caught tzdata-on-Windows + test-platform bugs. | ✅ | 2026-08-06 |
