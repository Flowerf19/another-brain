...
| TASK-095 | clean-tree gate false positive: the gate's forbidden pattern includes `compose` (Docker Compose era); the setup docstring/CHANGELOG/test docs used "composes/composition" → gate FAIL. Reword to neutral verbs ("runs/bundles/steps"); never weaken the pattern — it guards against the old stack returning. | | |
| TASK-096 | Bare `another-brain model` / `another-brain admin` exit 2 silently (no output) because the sub-subcommand is optional. Make the subparsers `required=True` so argparse prints the usage error; pin with tests. Found by real usage: `another-brain model` printed nothing. | | |
| TASK-097 | Ctrl-C on `serve` leaks a `threading._shutdown` KeyboardInterrupt traceback. Reproduce (`timeout -s INT`), then make shutdown quiet and typed (conventional exit 130); fix at the level that actually silences it (catch in main vs daemon threads — decide from the repro, not a guess). | | |
