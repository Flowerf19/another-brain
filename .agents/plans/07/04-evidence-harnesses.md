...
> Status 2026-08-05: the vector half landed with TASK-063 (both adapters,
> parity per the approved revision in 07.07). **The weighted-FTS5 half is
> still missing: `benchmarks/retrieval/run_suite.py` measures only
> `vector_branch` and `hybrid_search` — it never times the lexical branch.**
> Hand-measured p95 on the existing stores (100 warmups / 400 measured, same
> connect-included method the suite uses for the vector branch): 3.12 ms @1k,
> 8.90 ms @10k, 35.06 ms @50k, 71.68 ms @100k — versus sqlite-vec 1.85 /
> 2.98 / 7.75 / 13.53 ms and NumPy 3.22 / 5.32 / 13.03 / 26.90 ms. Lexical is
> the slowest branch at scale (5.3x sqlite-vec at 100k) and accounts for most
> of the 116.92 ms hybrid p95, yet Success criterion 9 sets budgets only for
> *vector* retrieval, so nothing gates it. Fold a `lexical_branch` timing
> series into the suite and decide whether a budget is warranted.
