...
| ID | Task | Done | Date |
|----|------|------|------|
| TASK-088 | Harness connectors (now at `installer/linux/connect.sh` + `installer/macos/` wrapper after the 2026-08-06 move): decide HTTP → stdio — connectors currently register `http://localhost:1905/mcp`, which assumes a running `serve --http` and contradicts the zero-server story; stdio (`{"command": "another-brain"}`) is also the only Windows-viable form. Windows story = documented MCP config JSON (identical content every OS), not a .ps1 port. Remove Docker/Redis/uvx assumptions. | | |
